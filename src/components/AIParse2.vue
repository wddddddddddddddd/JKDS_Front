<template>
  <div class="layout-sider-coll">
    ...
    <div collapsed-width=0 hide-trigger collapsible :collapsed="collapsed" :width="300" class="chat-sidebar">
      ...
      <div class="chat-assistant">
        ...
        <div class="chatRecordBox">
          <div v-for="(item, index) in chatRecordList" :key="index">
            <RightRole v-if="item.role === '访客'" :type="item.type" :content="item.content" :avatarURL="visitorAvatarURL"
              @openMenu="openMenu($event, index, true)" />
            <LeftRole v-if="item.role === '客服'" :type="item.type" :content="item.content" :avatarURL="serviceAvatarURL"
...
export default defineComponent({
  ...
  setup() {
    ...
    const onCollapse = () => {
      ...
      let sider = document.getElementsByClassName('arco-layout-sider-light')
      console.debug('side', sider, collapsed)
      if (!collapsed.value) {
        console.debug('side', sider)
        sider[0].offsetWidth = 0
      }
    };
